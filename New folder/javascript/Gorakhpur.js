const BASE_URL = `${BASE_URL_All}:8992/Gorakhpur`;
const GEOSERVER_BASE_URL = "http://localhost:8080/geoserver/GKP_Summary";
...
['Bank_Road', 'Park_Road', 'Hospital_Road', 'Education_Road', 'ShowRoads'].forEach(id => {
...
var zone_boundary = new ol.layer.Image({
    title: "Gorakhpur Zone Boundary",
    //  extent: [-180, -90, -180, 90],
    source: new ol.source.ImageWMS({
        url: `${GEOSERVER_BASE_URL}/wms?`,
        params: {
            LAYERS: "GKP_Summary:gorakhpur_zone_boundary",
        },
        ratio: 1,
...
var ward_boundary = new ol.layer.Image({
    title: "Gorakhpur Ward Boundary",
    //  extent: [-180, -90, -180, 90],
    source: new ol.source.ImageWMS({
        url: `${GEOSERVER_BASE_URL}/wms?`,
        params: {
            LAYERS: "GKP_Summary:gorakhpur_ward_boundary",
        },
        ratio: 1,
        serverType: "geoserver",
    }),
...
function clearAllVectorLayers() {
    ...
    map.getLayers().getArray().slice().forEach(layer => {
        if ((layer instanceof ol.layer.Tile || layer instanceof ol.layer.Image) &&
            ...
            if (layer.get('title') !== 'Gorakhpur Zone Boundary' && layer.get('title') !== 'Gorakhpur Ward Boundary') {
...
function zoomToIndia() {
    ...
    view.setCenter([83.3865, 26.7300]);
    view.setZoom(12.5);
    view.setRotation(0);
...
const locationTypes = [
    ...
    { type: 'car_charging', icon: '../assets/icons/charging.png', id: 'showCar' },
    ...
    { type: 'Police_Station', icon: '../assets/icons/police-station (2).png', id: 'showPoliceStation' },
    { type: 'communication', icon: '../assets/icons/communication.png', id: 'showCommunication' },
                ...
                responseData.data.forEach(item => {
                    ...
                    row.innerHTML = `
                         ...
                         <td>${item.row_meter}</td>
                            ...
                            <td>${item.material}</td>
                <td>${item.length_km.toFixed(2)}</td>
...
$(document).ready(function () {
    $.ajax({
        ...
        success: function (xml) {
            var select = $("#layer");
            const targetLayer = "GKP_Summary:gorakhpur_road_net"; // your desired layer
            $(xml)
                .find("FeatureType")
                .each(function () {
...
$(document).ready(function () {
    $.ajax({
        ...
        success: function (xml) {
            var select = $("#layer1");
            var targetWorkspace = "GKP_Summary:gorakhpur_road_net"; // :point_left: workspace prefix
            $(xml)
                .find("FeatureType")
                .each(function () {
...
function query() {
    ...
    $.getJSON(url, function (data) {
       ...
       var table = document.createElement("table");
        ...
        caption.innerHTML =
            // value_layer +
            " (<b style='font-weight: 900;color: rgb(0, 0, 0);font-size: small;'>Number of Features : " + data.features.length + " )";
...
function updateSummary(data) {
    ...
    const summaryData = {
        'No. of Zones': { value: safeValue('zone_count'), onclick: 'Gorakhpur_Zone_no()' },
        'Total Road Length': { value: `${safeValue('total_length_km')} km`, onclick: "Gorakhpur_Road_Length_Count(); updateNavBarWithFunctionName('Total Road Length');" },
        'Total No. of Roads': { value: safeValue('total_length_count'), onclick: "Gorakhpur_Road_Length_Count(); updateNavBarWithFunctionName('Total Road Count');" },
        'Total Ward No.': { value: safeValue('ayo_ward'), onclick: 'Gorakhpur_Ward_NO()' },
        'Road Count by Condition': {
            value: `
                Good - <a href="javascript:void(0)" onclick="Gorakhpur_Condition_cat('condition','Good'); updateNavBarWithFunctionName('Road Condition : Good');" style="color:green;">${safeValue('condition_count_green')}</a><br>
                Poor - <a href="javascript:void(0)" onclick="Gorakhpur_Condition_cat('condition','Poor'); updateNavBarWithFunctionName('Road Condition : Poor');" style="color:red;">${safeValue('condition_count_red')}</a><br>
                Moderate - <a href="javascript:void(0)" onclick="Gorakhpur_Condition_cat('condition','Moderate'); updateNavBarWithFunctionName('Road Condition : Moderate');" style="color:yellow;">${safeValue('condition_count_yellow')}</a><br>
                NA - <a href="javascript:void(0)" onclick="Gorakhpur_Condition_cat('condition','NA'); updateNavBarWithFunctionName('Road Condition : NA');" style="color:pink;">${safeValue('condition_count_na')}</a><br>
            `
        },

        'Road Count by Material': {
            value: `
                Bitumen - <a href="javascript:void(0)" onclick="Gorakhpur_Material_cat('material','Bitumen'); updateNavBarWithFunctionName('Road Material : Bitumen');" style="color:darkred;">${safeValue('count_bitumen')}</a><br>
                CC - <a href="javascript:void(0)" onclick="Gorakhpur_Material_cat('material','CC'); updateNavBarWithFunctionName('Road Material : CC');" style="color:#1ad7b0;">${safeValue('count_cc')}</a><br>
                Interlocking - <a href="javascript:void(0)" onclick="Gorakhpur_Material_cat('material','Interlocking'); updateNavBarWithFunctionName('Road Material : Interlocking');" style="color:#2392ed;">${safeValue('count_interlocking')}</a><br>
                BOE - <a href="javascript:void(0)" onclick="Gorakhpur_Material_cat('material','BOE'); updateNavBarWithFunctionName('Road Material : BOE');" style="color:#f228ab;">${safeValue('count_boe')}</a><br>
                Kachcha - <a href="javascript:void(0)" onclick="Gorakhpur_Material_cat('material','Kachcha'); updateNavBarWithFunctionName('Road Material : Kachcha');" style="color:#6036d0;">${safeValue('count_kachcha')}</a><br>
                NA - <a href="javascript:void(0)" onclick="Gorakhpur_Material_cat('material','NA'); updateNavBarWithFunctionName('Road Material : NA');" style="color:#dfc223;">${safeValue('count_na')}</a>
                `,
            onclick: 'Gorakhpur_Material()'
        },
        'Road Count by Ownership': {
            value: `
                GKPNN - <a href="javascript:void(0)" onclick="Gorakhpur_Ownership_cat('ownership','GKPNN'); updateNavBarWithFunctionName('Road Ownership : Gorakhpur Nagar Nigam');" style="color:#5aeee5;">${safeValue('count_gkpnn')}</a><br>
                PWD - <a href="javascript:void(0)" onclick="Gorakhpur_Ownership_cat('ownership','PWD'); updateNavBarWithFunctionName('Road Ownership : PWD');" style="color:#69e70f;">${safeValue('count_pwd')}</a><br>
                Private Road - <a href="javascript:void(0)" onclick="Gorakhpur_Ownership_cat('ownership','Private'); updateNavBarWithFunctionName('Road Ownership : Private Roads');" style="color:#ed2323;">${safeValue('count_pvt')}</a><br>
                NHAI - <a href="javascript:void(0)" onclick="Gorakhpur_Ownership_cat('ownership','NHAI'); updateNavBarWithFunctionName('Road Ownership : National Highways Authority of India');" style="color:rgb(33,14,139);">${safeValue('count_nhai')}</a><br>
                Railway - <a href="javascript:void(0)" onclick="Gorakhpur_Ownership_cat('ownership','Railway'); updateNavBarWithFunctionName('Road Ownership : Railway');" style="color:yellow;">${safeValue('count_railway')}</a><br>
                C M Grid - <a href="javascript:void(0)" onclick="Gorakhpur_Ownership_cat('ownership','C.M Grid'); updateNavBarWithFunctionName('Road Ownership : C.M Grid');" style="color:#f16a16;">${safeValue('count_grid')}</a><br>
                Defence - <a href="javascript:void(0)" onclick="Gorakhpur_Ownership_cat('ownership','Defence'); updateNavBarWithFunctionName('Road Ownership : Defence');" style="color:#ecc82e;">${safeValue('count_defence')}</a><br>
                Department Roads - <a href="javascript:void(0)" onclick="Gorakhpur_Ownership_cat('ownership','Department Roads'); updateNavBarWithFunctionName('Road Ownership : Department Roads');" style="color:#8d139b;">${safeValue('count_department_roads')}</a><br>
                Institutional Roads - <a href="javascript:void(0)" onclick="Gorakhpur_Ownership_cat('ownership','Institutional Roads'); updateNavBarWithFunctionName('Road Ownership : Institutional Roads');" style="color:#168de7;">${safeValue('count_institutional_roads')}</a><br>
                Development Authority - <a href="javascript:void(0)" onclick="Gorakhpur_Ownership_cat('ownership','Development Authority'); updateNavBarWithFunctionName('Road Ownership : Development Authority');" style="color:#831042;">${safeValue('count_development_authority')}</a>         
                `,
            onclick: 'Gorakhpur_Ownership()'
        },
        'Road Count by CUS': {
            value: `
                14th Finance - <a href="javascript:void(0)" onclick="Gorakhpur_CUS_cat('cus','14th Finance'); updateNavBarWithFunctionName('Road CUS : 14th Finance');" style="color:#ec1248;">${safeValue('count_cus_14th')}</a><br>
                15th Finance - <a href="javascript:void(0)" onclick="Gorakhpur_CUS_cat('cus','15th Finance'); updateNavBarWithFunctionName('Road CUS : 15th Finance');" style="color:#e63dee;">${safeValue('count_cus_15th')}</a><br>
                Nagar Nigam Nidhi - <a href="javascript:void(0)" onclick="Gorakhpur_CUS_cat('cus','Nagar Nigam Nidhi'); updateNavBarWithFunctionName('Road CUS : Nagar Nigam Nidhi');" style="color:cyan;">${safeValue('count_cus_nnn')}</a><br>
                DUDA - <a href="javascript:void(0)" onclick="Gorakhpur_CUS_cat('cus','DUDA'); updateNavBarWithFunctionName('Road CUS : DUDA');" style="color:#e1ca4c;">${safeValue('count_cus_duda')}</a><br>
                GDA - <a href="javascript:void(0)" onclick="Gorakhpur_CUS_cat('cus','GDA'); updateNavBarWithFunctionName('Road CUS : GDA');" style="color:#ec1248;">${safeValue('count_cus_gda')}</a><br>
                NA - <a href="javascript:void(0)" onclick="Gorakhpur_CUS_cat('cus','NA'); updateNavBarWithFunctionName('Road CUS : NA');" style="color:cyan;">${safeValue('count_cus_na')}</a><br>
                PWD - <a href="javascript:void(0)" onclick="Gorakhpur_CUS_cat('cus','PWD'); updateNavBarWithFunctionName('Road CUS : PWD');" style="color:#e1ca4c;">${safeValue('count_cus_pwd')}</a><br>
                Others - <a href="javascript:void(0)" onclick="Gorakhpur_CUS_cat('cus_class','Others'); updateNavBarWithFunctionName('Road CUS : Others');" style="color:#e1ca4c;">${safeValue('count_cus_others')}</a><br>

                `
            ,
            onclick: 'Gorakhpur_CUS()'
        },
        'Road Count by Category': {
            value: `
                Local Street - <a href="javascript:void(0)" onclick="Gorakhpur_TypeSub_cat('category','Local Street'); updateNavBarWithFunctionName('Road Category : Local Street');" style="color: #14cee3;">${safeValue('count_local_street')}</a><br>
                Collector - <a href="javascript:void(0)" onclick="Gorakhpur_TypeSub_cat('category','Collector'); updateNavBarWithFunctionName('Road Category : Collector');" style="color: #e63dee;">${safeValue('count_collector')}</a><br>
                Arterial - <a href="javascript:void(0)" onclick="Gorakhpur_TypeSub_cat('category','Arterial'); updateNavBarWithFunctionName('Road Category : Arterial');" style="color: #e1ca4c;">${safeValue('count_arterial')}</a><br>
                Sub Arterial - <a href="javascript:void(0)" onclick="Gorakhpur_TypeSub_cat('category','Sub Arterial'); updateNavBarWithFunctionName('Road Category : Sub Arterial');" style="color: #83e45c;">${safeValue('count_subarterial')}</a><br>
                `
            ,
        }
    };
    ...
    const zoneCards = [
        {
            ...
            value: `<a href="javascript:void(0)" onclick="Gorakhpur_Zone_no('zone_no','${zoneNo}'); updateNavBarWithFunctionName('Zone-${zoneNo} Total Road Length');" style="color:black;">${data.length_km.toFixed(2)} km</a>`
        },
        {
            title: 'Total No. of Roads',
            value: `<a href="javascript:void(0)" onclick="Gorakhpur_Zone_no('zone_no','${zoneNo}'); updateNavBarWithFunctionName('Zone-${zoneNo} Total Road Count');" style="color:black;">${data.total_no_of_roads}</a>`
        },
        // { 
        //     title: 'Road Type', 
        //     value: `Major <a href="javascript:void(0)" onclick="Gorakhpur_Zone_Type('${zoneNo}','type','Major City Road'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Type Major');" style="color:blue;">- ${data.total_major_sum}</a> <br> 
        //             Minor <a href="javascript:void(0)" onclick="Gorakhpur_Zone_Type('${zoneNo}','type','Minor City Road'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Type Minor');" style="color:yellow;">- ${data.total_minor_sum}</a>`
        // },
        {
            title: 'Road Condition',
            value: `
                    Good <a href="javascript:void(0)" onclick="Gorakhpur_Zone_Condition('${zoneNo}','condition','Good'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Condition : Good');" style="color:green;">- ${data.count_green}</a> <br> 
                    Moderate <a href="javascript:void(0)" onclick="Gorakhpur_Zone_Condition('${zoneNo}','condition','Moderate'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Condition : Moderate');" style="color:yellow;">- ${data.count_yellow}</a> <br> 
                    Poor <a href="javascript:void(0)" onclick="Gorakhpur_Zone_Condition('${zoneNo}','condition','Poor'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Condition : Poor');" style="color:red;">- ${data.count_red}</a><br>
                    NA <a href="javascript:void(0)" onclick="Gorakhpur_Zone_Condition('${zoneNo}','condition','NA'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Condition : NA');" style="color:pink;">- ${data.count_na}</a> <br> 
                    `
        },
        {
            title: 'Materials',
            value: `
                    Bitumen <a href="javascript:void(0)" onclick="Gorakhpur_Zone_Material('${zoneNo}','material','Bitumen'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Material : Bitumen');" style="color:darkred;">- ${data.bitumen}</a> <br>
                    CC <a href="javascript:void(0)" onclick="Gorakhpur_Zone_Material('${zoneNo}','material','CC'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Material : CC');" style="color:#1ad7b0;">- ${data.cc}</a> <br>
                    Interlocking <a href="javascript:void(0)" onclick="Gorakhpur_Zone_Material('${zoneNo}','material','Interlocking'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Material : Interlocking');" style="color:#2392ed;">- ${data.interlocking}</a> <br>
                    BOE <a href="javascript:void(0)" onclick="Gorakhpur_Zone_Material('${zoneNo}','material','BOE'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Material : BOE');" style="color:#f228ab;">- ${data.boe}</a> <br>
                    Kachcha <a href="javascript:void(0)" onclick="Gorakhpur_Zone_Material('${zoneNo}','material','Kachcha'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Material : Kachcha');" style="color:#6036d0;">- ${data.kachcha}</a><br>
                    NA - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_Material('${zoneNo}','material','NA'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Material : NA');" style="color:#dfc223;">${data.material_na}</a>
                    `
        },
        {
            title: 'Ownership',
            value: `
                    GKPNN - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_Ownership('${zoneNo}','ownership','GKPNN'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership : Gorakhpur Nagar Nigam');" style="color:#5aeee5;">${data.count_gkpnn}</a><br>
                    PWD - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_Ownership('${zoneNo}','ownership','PWD'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership : PWD');" style="color:#69e70f;">${data.count_pwd}</a><br>
                    Private Road - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_Ownership('${zoneNo}','ownership','Private'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership : Private Roads');" style="color:#ed2323;">${data.count_pvt}</a><br>
                    NHAI - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_Ownership('${zoneNo}','ownership','NHAI'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership : National Highways Authority of India');" style="color:rgb(33,14,139);">${data.count_nhai}</a><br>
                    Railway - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_Ownership('${zoneNo}','ownership','Railway'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership : Railway');" style="color:yellow;">${data.count_railway}</a><br>
                    C M Grid - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_Ownership('${zoneNo}','ownership','C.M Grid'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership : C.M Grid');" style="color:#f16a16;">${data.count_grid}</a><br>
                    Defence - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_Ownership('${zoneNo}','ownership','Defence'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership : Defence');" style="color:#ecc82e;">${data.count_defence}</a><br>
                    Department Roads - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_Ownership('${zoneNo}','ownership','Department Roads'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership : Department Roads');" style="color:#8d139b;">${data.count_department_roads}</a><br>
                    Institutional Roads - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_Ownership('${zoneNo}','ownership','Institutional Roads'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership : Institutional Roads');" style="color:#168de7;">${data.count_institutional_roads}</a><br>
                    Development Authority - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_Ownership('${zoneNo}','ownership','Development Authority'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership : Development Authority');" style="color:#831042;">${data.count_development_authority}</a>         
                    `
        },
        {
            title: 'CUS',
            value: ` 
                    14th Finance - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_CUS('${zoneNo}','cus','14th Finance'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS : 14th Finance');" style="color:#ec1248;">${data.count_cus_14th}</a><br>
                    15th Finance - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_CUS('${zoneNo}','cus','15th Finance'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS : 15th Finance');" style="color:#e63dee;">${data.count_cus_15th}</a><br>
                    Nagar Nigam Nidhi - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_CUS('${zoneNo}','cus','Nagar Nigam Nidhi'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS : Nagar Nigam Nidhi');" style="color:cyan;">${data.count_cus_nnn}</a><br>
                    DUDA - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_CUS('${zoneNo}','cus','DUDA'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS : DUDA');" style="color:#e1ca4c;">${data.count_cus_duda}</a><br>
                    GDA - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_CUS('${zoneNo}','cus','GDA'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS : GDA');" style="color:#ec1248;">${data.count_cus_gda}</a><br>
                    NA - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_CUS('${zoneNo}','cus','NA'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS : NA');" style="color:cyan;">${data.count_cus_na}</a><br>
                    PWD - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_CUS('${zoneNo}','cus','PWD'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS : PWD');" style="color:#e1ca4c;">${data.count_cus_pwd}</a><br>
                    Others - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_CUS('${zoneNo}','cus_class','Others'); updateNavBarWithFunctionName(''Zone-${zoneNo} Road CUS : Others');" style="color:#e1ca4c;">${data.count_cus_others}</a><br>
  `
        },
        {
            title: 'Type Sub Category',
            value: `
                    Local Street - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_TypeSub('${zoneNo}','category','Local Street'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Category : Local Street');" style="color: #14cee3;">${data.local_street} </a><br>
                    Collector - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_TypeSub('${zoneNo}','category','Collector'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Category : Collector');" style="color: #e63dee;">${data.collector}</a><br>
                    Arterial - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_TypeSub('${zoneNo}','category','Arterial'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Category : Arterial');" style="color: #e1ca4c;">${data.arterial}</a><br>
                    Sub Arterial - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_TypeSub('${zoneNo}','category','Sub Arterial'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Category : Sub Arterial');" style="color: #83e45c;">${data.subarterial}</a><br>
 `
        },
    ];
        ...
        .then(responseData => {
            if (responseData.status) {
                ...
                const details = [
                    // { 
                    //     title: 'Road Type', 
                    //     value: `Major - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_Type('${wardNo}', 'type', 'Major City Road'); updateNavBarWithFunctionName('Ward-${wardNo} Road Type Major');" style="color:blue;">${responseData.total_major_sum}</a><br> 
                    //              Minor - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_Type('${wardNo}', 'type', 'Minor City Road'); updateNavBarWithFunctionName('Ward-${wardNo} Road Type Minor');" style="color:orange;">${responseData.total_minor_sum}</a>` 
                    // },
                    {
                        title: 'Total Road Length',
                        value: `<a href="javascript:void(0)" onclick="Gorakhpur_Ward_no('ward_no', '${wardNo}'); updateNavBarWithFunctionName('Ward-${wardNo} Total Road Length');" style="color:black;">${responseData.length_km} km</a>`
                    },
                    {
                        title: 'Total No. of Roads',
                        value: `<a href="javascript:void(0)" onclick="Gorakhpur_Ward_no('ward_no', '${wardNo}'); updateNavBarWithFunctionName('Ward-${wardNo} Total Road Count');" style="color:black;">${responseData.total_no_of_roads}</a>`
                    },
                    {
                        title: 'Road Condition',
                        value: `
                                Good - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_Condition('${wardNo}', 'condition', 'Good'); updateNavBarWithFunctionName('Ward-${wardNo} Road Condition : Good');" style="color:green;">${responseData.count_green}</a><br> 
                                Moderate - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_Condition('${wardNo}', 'condition', 'Moderate'); updateNavBarWithFunctionName('Ward-${wardNo} Road Condition : Moderate');" style="color:yellow;">${responseData.count_yellow}</a><br> 
                                Poor - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_Condition('${wardNo}', 'condition', 'Poor'); updateNavBarWithFunctionName('Ward-${wardNo} Road Condition : Poor');" style="color:red;">${responseData.count_red}</a><br>
                                NA - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_Condition('${wardNo}', 'condition', 'NA'); updateNavBarWithFunctionName('Ward-${wardNo} Road Condition : NA');" style="color:pink;">${responseData.count_na}</a>
                                `
                    },
                    {
                        title: 'Materials',
                        value: `
                                Bitumen - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_Material('${wardNo}', 'material', 'Bitumen'); updateNavBarWithFunctionName('Ward-${wardNo} Road Material : Bitumen');" style="color:darkred;">${responseData.bitumen}</a><br> 
                                CC - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_Material('${wardNo}', 'material', 'CC'); updateNavBarWithFunctionName('Ward-${wardNo} Road Material : CC');" style="color:cyan;">${responseData.cc}</a><br> 
                                Interlocking - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_Material('${wardNo}', 'material', 'Interlocking'); updateNavBarWithFunctionName('Ward-${wardNo} Road Material : Interlocking');" style="color:blue;">${responseData.interlocking}</a><br>
                                BOE - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_Material('${wardNo}', 'material', 'BOE'); updateNavBarWithFunctionName('Ward-${wardNo} Road Material : BOE');" style="color:pink;">${responseData.boe}</a><br> 
                                Kachcha - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_Material('${wardNo}', 'material', 'Kachcha'); updateNavBarWithFunctionName('Ward-${wardNo} Road Material : Kachcha');" style="color:purple;">${responseData.kachcha}</a><br>
                                NA - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_Material('${wardNo}','material','NA'); updateNavBarWithFunctionName('Ward-${wardNo} Road Material : NA');" style="color:#dfc223;">${responseData.material_na}</a>
                                `
                    },
                    {
                        title: 'Ownership',
                        value: `
                                GKPNN - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_Ownership('${wardNo}','ownership','GKPNN'); updateNavBarWithFunctionName('Zone-${wardNo} Road Ownership : Gorakhpur Nagar Nigam');" style="color:#5aeee5;">${responseData.count_gkpnn}</a><br>
                                PWD - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_Ownership('${wardNo}','ownership','PWD'); updateNavBarWithFunctionName('Zone-${wardNo} Road Ownership : PWD');" style="color:#69e70f;">${responseData.count_pwd}</a><br>responseData
                                Private Road - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_Ownership('${wardNo}','ownership','Private'); updateNavBarWithFunctionName('Zone-${wardNo} Road Ownership : Private Roads');" style="color:#ed2323;">${responseData.count_pvt}</a><br>
                                NHAI - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_Ownership('${wardNo}','ownership','NHAI'); updateNavBarWithFunctionName('Zone-${wardNo} Road Ownership : National Highways Authority of India');" style="color:rgb(33,14,139);">${responseData.count_nhai}</a><br>
                                Railway - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_Ownership('${wardNo}','ownership','Railway'); updateNavBarWithFunctionName('Zone-${wardNo} Road Ownership : Railway');" style="color:yellow;">${responseData.count_railway}</a><br>
                                C M Grid - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_Ownership('${wardNo}','ownership','C.M Grid'); updateNavBarWithFunctionName('Zone-${wardNo} Road Ownership : C.M Grid');" style="color:#f16a16;">${responseData.count_grid}</a><br>
                                Defence - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_Ownership('${wardNo}','ownership','Defence'); updateNavBarWithFunctionName('Zone-${wardNo} Road Ownership : Defence');" style="color:#ecc82e;">${responseData.count_defence}</a><br>
                                Department Roads - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_Ownership('${wardNo}','ownership','Department Roads'); updateNavBarWithFunctionName('Zone-${wardNo} Road Ownership : Department Roads');" style="color:#8d139b;">${responseData.count_department_roads}</a><br>
                                Institutional Roads - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_Ownership('${wardNo}','ownership','Institutional Roads'); updateNavBarWithFunctionName('Zone-${wardNo} Road Ownership : Institutional Roads');" style="color:#168de7;">${responseData.count_institutional_roads}</a><br>
                                Development Authority - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_Ownership('${wardNo}','ownership','Development Authority'); updateNavBarWithFunctionName('Zone-${wardNo} Road Ownership : Development Authority');" style="color:#831042;">${responseData.count_development_authority}</a>         
                                `
                    },
                    {
                        title: 'CUS',
                        value: `
                                14th Finance - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_CUS('${wardNo}','cus','14th Finance'); updateNavBarWithFunctionName('Zone-${wardNo} Road CUS : 14th Finance');" style="color:#ec1248;">${responseData.count_cus_14th}</a><br>
                                15th Finance - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_CUS('${wardNo}','cus','15th Finance'); updateNavBarWithFunctionName('Zone-${wardNo} Road CUS : 15th Finance');" style="color:#e63dee;">${responseData.count_cus_15th}</a><br>
                                Nagar Nigam Nidhi - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_CUS('${wardNo}','cus','Nagar Nigam Nidhi'); updateNavBarWithFunctionName('Zone-${wardNo} Road CUS : Nagar Nigam Nidhi');" style="color:cyan;">${responseData.count_cus_nnn}</a><br>
                                DUDA - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_CUS('${wardNo}','cus','DUDA'); updateNavBarWithFunctionName('Zone-${wardNo} Road CUS : DUDA');" style="color:#e1ca4c;">${responseData.count_cus_duda}</a><br>
                                GDA - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_CUS('${wardNo}','cus','GDA'); updateNavBarWithFunctionName('Zone-${wardNo} Road CUS : GDA');" style="color:#ec1248;">${responseData.count_cus_gda}</a><br>
                                NA - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_CUS('${wardNo}','cus','NA'); updateNavBarWithFunctionName('Zone-${wardNo} Road CUS : NA');" style="color:cyan;">${responseData.count_cus_na}</a><br>
                                PWD - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_CUS('${wardNo}','cus','PWD'); updateNavBarWithFunctionName('Zone-${wardNo} Road CUS : PWD');" style="color:#e1ca4c;">${responseData.count_cus_pwd}</a><br>
                                Others - <a href="javascript:void(0)" onclick="Gorakhpur_Zone_CUS('${wardNo}','cus_class','Others'); updateNavBarWithFunctionName(''Zone-${wardNo} Road CUS : Others');" style="color:#e1ca4c;">${responseData.count_cus_others}</a><br>
 `  },
                    {
                        title: 'Type Sub Category',
                        value: `
                                Local Street - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_TypeSub('${wardNo}','category','Local Street'); updateNavBarWithFunctionName('Ward-${wardNo} Road Category : Local Street');" style="color: #14cee3;">${responseData.local_street} </a><br>
                                Collector - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_TypeSub('${wardNo}','category','Collector'); updateNavBarWithFunctionName('Ward-${wardNo} Road Category : Collector');" style="color: #e63dee;">${responseData.collector}</a><br>
                                Arterial - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_TypeSub('${wardNo}','category','Arterial'); updateNavBarWithFunctionName('Ward-${wardNo} Road Category : Arterial');" style="color: #e1ca4c;">${responseData.arterial}</a><br>
                                Sub Arterial - <a href="javascript:void(0)" onclick="Gorakhpur_Ward_TypeSub('${wardNo}','category','Sub Arterial'); updateNavBarWithFunctionName('Ward-${wardNo} Road Category : Sub Arterial');" style="color: #83e45c;">${responseData.subarterial}</a><br>
                               `},
                ];
...
function highlightFeatureOnMap(gisId) {
    let wfsUrl = `${GEOSERVER_BASE_URL}/wfs?service=WFS&version=1.1.0&request=GetFeature
        &typename=GKP_Summary:gorakhpur_road_net
...
function Gorakhpur_road_Length_Count() {
    removeCurrentLayer();
    clearVectorLayers();
    ...
    currentLayer = new ol.layer.Image({
        ...
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GKP_Summary:gorakhpur_road_net',
...
function Gorakhpur_Ownership(cqlFilter) {
    removeCurrentLayer(); clearVectorLayers();
    currentLayer = new ol.layer.Image({
        //   title: 'Ward Boundary',
        //     extent: [-180, -90, -180, 90],
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GKP_Summary:gorakhpur_road_own',
...
function Gorakhpur_Material(cqlFilter) {
    removeCurrentLayer(); clearVectorLayers();
    currentLayer = new ol.layer.Image({
        //   title: 'Ward Boundary',
        //     extent: [-180, -90, -180, 90],
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GKP_Summary:gorakhpur_road_mat',
                'CQL_FILTER': cqlFilter,
            },
            ratio: 1,
            serverType: 'geoserver'
...
function Gorakhpur_Types(cqlFilter) {
    removeCurrentLayer(); clearVectorLayers();
    currentLayer = new ol.layer.Image({
        //   title: 'Ward Boundary',
        //     extent: [-180, -90, -180, 90],
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GKP_Summary:gorakhpur_road_category',
                'CQL_FILTER': cqlFilter,
            },
            ratio: 1,
            serverType: 'geoserver'
...
//$(document).ready(function () {
  ...
  $(document).ready(function () {
    ...
    var wfsParams = {
        ...
        typeName: 'GKP_Summary:gorakhpur_road_net',   // Replace with your WFS layer typename
    ...
    const url = `${GEOSERVER_BASE_URL}/wfs?service=WFS&version=1.0.0&request=GetFeature&typeName=GKP_Summary:gorakhpur_road_net&outputFormat=application/json`;
...
function Gorakhpur_Zone_no(column, value) {
    removeCurrentLayer();
    // clearVectorLayers();
...
function Gorakhpur_Ward_no(column, value) {
    removeCurrentLayer();
    // clearVectorLayers();
...
function Gorakhpur_Condition_cat(column, value) {
    removeCurrentLayer();
    ///  clearVectorLayers();
    // Define dynamic CQL filter
    let cqlFilter = `(${column}='${value}' OR ${column} ILIKE '%/${value}' OR ${column} ILIKE '${value}/%' OR ${column} ILIKE '%/${value}/%' OR ${column} ILIKE '${value}')`;
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GKP_Summary:gorakhpur_road_condition',   // Same layer for all filters
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Gorakhpur_Material_cat(column, value) {
    removeCurrentLayer();
    //  clearVectorLayers();
    // Define dynamic CQL filter
    let cqlFilter = `(${column}='${value}' OR ${column} ILIKE '%/${value}' OR ${column} ILIKE '${value}/%' OR ${column} ILIKE '%/${value}/%' OR ${column} ILIKE '${value}')`;
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GKP_Summary:gorakhpur_road_mat',  // Same layer for all filters
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Gorakhpur_Ownership_cat(column, value) {
    removeCurrentLayer();
    //  clearVectorLayers();
    // Define dynamic CQL filter
    let cqlFilter = `(${column}='${value}' OR ${column} ILIKE '%/${value}' OR ${column} ILIKE '${value}/%' OR ${column} ILIKE '%/${value}/%' OR ${column} ILIKE '${value}')`;
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GKP_Summary:gorakhpur_road_own',   // Same layer for all filters
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Gorakhpur_CUS_cat(column, value) {
    removeCurrentLayer();
    // clearVectorLayers();
    // Define dynamic CQL filter
    let cqlFilter = `(${column}='${value}' OR ${column} ILIKE '%/${value}' OR ${column} ILIKE '${value}/%' OR ${column} ILIKE '%/${value}/%' OR ${column} ILIKE '${value}')`;
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GKP_Summary:gorakhpur_road_cus',   // Same layer for all filters
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Gorakhpur_TypeSub_cat(column, value) {
    removeCurrentLayer();
    // clearVectorLayers();
    // Define dynamic CQL filter
    let cqlFilter = `(${column}='${value}' OR ${column} ILIKE '%/${value}' OR ${column} ILIKE '${value}/%' OR ${column} ILIKE '%/${value}/%' OR ${column} ILIKE '${value}')`;
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GKP_Summary:gorakhpur_road_category',   // Same layer for all filters
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Gorakhpur_Zone_Condition(zone_no, column, value) {
    removeCurrentLayer();
    //  clearVectorLayers();
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GKP_Summary:gorakhpur_road_condition',
                'CQL_FILTER': cqlFilter,
                'TILED': true
            },
            ratio: 1,
...
function Gorakhpur_Zone_Material(zone_no, column, value) {
    removeCurrentLayer();
    //  clearVectorLayers();
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GKP_Summary:gorakhpur_road_mat',   // Main layer for filtering
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Gorakhpur_Zone_Ownership(zone_no, column, value) {
    removeCurrentLayer();
    // clearVectorLayers();
    // Ensure column is valid to prevent errors
    let validColumns = ["condition", "category", "material", "ownership", "cus","cus_class"];
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GKP_Summary:gorakhpur_road_own',   // Main layer for filtering
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Gorakhpur_Zone_CUS(zone_no, column, value) {
    removeCurrentLayer();
    // clearVectorLayers();
    // Ensure column is valid to prevent errors
    let validColumns = ["condition", "category", "material", "ownership", "cus","cus_class"];
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GKP_Summary:gorakhpur_road_cus',   // Main layer for filtering
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Gorakhpur_Zone_TypeSub(zone_no, column, value) {
    removeCurrentLayer();
    clearVectorLayers();
    // Ensure column is valid to prevent errors
    let validColumns = ["condition", "category", "material", "ownership", "cus","cus_class",];
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GKP_Summary:gorakhpur_road_category',   // Main layer for filtering
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Gorakhpur_Ward_Condition(ward_no, column, value) {
    removeCurrentLayer();
    //  clearVectorLayers();
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'GKP_Summary:gorakhpur_road_condition', // Main layer for filtering
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Gorakhpur_Ward_Material(ward_no, column, value) {
    removeCurrentLayer();
    // clearVectorLayers();
    // Ensure column is valid to prevent errors
    let validColumns = ["condition", "category", "material", "ownership", "cus","cus_class"];
...
function Gorakhpur_Ward_Ownership(ward_no, column, value) {
    removeCurrentLayer();
    //  clearVectorLayers();
...
function Gorakhpur_Ward_CUS(ward_no, column, value) {
    removeCurrentLayer();
    // clearVectorLayers();
...
function Gorakhpur_Ward_TypeSub(ward_no, column, value) {
    removeCurrentLayer();
    clearVectorLayers();
    // Ensure column is valid to prevent errors
    let validColumns = ["condition", "category", "material", "ownership", "cus","cus_class"];
...
function determineLayer() {
    if (selectedFilters.material) {
        return 'GKP_Summary:gorakhpur_road_mat';
    } else if (selectedFilters.type) {
        return 'GKP_Summary:gorakhpur_road_category';
    } else if (selectedFilters.ownership) {
        return 'GKP_Summary:gorakhpur_road_own';
    }
    return 'GKP_Summary:gorakhpur_road_category';
}
...
function updateTable(layer, cqlFilter) {
 ...
 <div class="row-line"><strong>Length (Km) :</strong> ${
		      properties.length_km !== null && properties.length_km !== undefined
		          ? Number(properties.length_km).toFixed(2)
		          : 'N/A'
		  }</div>                            <td>${properties.condition || 'N/A'}</td>

const BASE_URL = `${BASE_URL_All}:8060/Ayodhya`;
const GEOSERVER_BASE_URL  = `${GEOSERVER_URL_All}/ANN_Summary`;
...
function showTables() {
    ...
    hideElements([ 'live_legend',  'tableContainer_Drain', 'dataTable_Drain',
...
function showTables1() {
    ...
    hideElements([
        'dataTable_summary', 'tableContainer_summary',  'summary-table',
        'live_legend',  'Condition_legend', 'Material_legend', 'Ownership_legend', //'Priority_legend','Scoreing_tableContainer', 'Scoring_dataTable',
        'tableContainer_summary',  'tableContainer_summaryfilter', 
    ]);
...
function showTables2() {
    ...
    // document.getElementById("main-container").style.height = "98%"; 
    document.getElementById("map").style.height = "100%";
    document.getElementById("table_data").style.height = "0%";
    showElements(['summary-table']);
    hideElements([  'tableContainer_summary','dataTable_summary',
        'tableContainer_Drain', 'dataTable_Drain', 'legendBtn', 'live_legend',  'Condition_legend',
        'Material_legend', 'Ownership_legend', // 'Priority_legend', 'Scoring_dataTable', 'Scoreing_tableContainer',
        'Drain_Condition_Legend', 'Drain_Material_Legend', 'Drain_Status_Legend','Drain_Type_Legend',
...
//     hideElements([
//  'dataTable_summary', 'tableContainer_summary', 'tableContainer_Drain', 
//          'tableContainer_summaryfilter', 
//         'Ownership_legend', 'Material_legend', 'Condition_legend', 
...
var zone_boundary = new ol.layer.Image({
    ...
    source: new ol.source.ImageWMS({
        ...
        params: {
            LAYERS: "ANN_Summary:ayodhya_zone_boundary",
        },
        ratio: 1,
        serverType: "geoserver",
    }),
});
//overlays.getLayers().push(ANN_Boundary);
...
var ward_boundary = new ol.layer.Image({
    ...
    source: new ol.source.ImageWMS({
        url: `${GEOSERVER_BASE_URL}/wms?`,
        params: {
            LAYERS: "ANN_Summary:ayodhya_ward_boundary",
        },
        ratio: 1,
        serverType: "geoserver",
    }),
...
function minimize1() {
	...
	const topnav1 = document.getElementById('tableContainer_summaryfilter');
...
function maximize1() {
    const topnav = document.getElementById('tableContainer_summary');
	//const topnav1 = document.getElementById('tableContainer');
	const navElements = document.querySelectorAll('.feature_nav');
	 //  const selection_filter = document.getElementById('dataTable_Road_Age');
	//   const selection_filter1 = document.getElementById('tableContainer_Road_Age');
    document.querySelectorAll('.feature_nav').forEach(nav => {
    ...
    topnav.style.height = '23%';
	//selection_filter.style.height = '29%'; // Reduced height when minimized
	 //  selection_filter1.style.height = '23%'; // Reduced height when minimized
	 //  topnav1.style.height = '27%';
...
ShowDrainage.addEventListener('click', function () {
    ...
    fetch(`${BASE_URL}/getAllDrainName`, {
        ...
        legendBtn.addEventListener('click', function () {
            const legends = [ live_legend, Condition_legend, Material_legend, Ownership_legend, // Priority_legend,
...
function priorityBasedScoring(priority, label) {
    ...
    fetch(`${BASE_URL}/getPriorityName?priority=${encodeURIComponent(priority)}`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
function handleLegendDisplay(priority) {
    // Hide all legends
    [ live_legend, Condition_legend, Material_legend, Ownership_legend, Priority_legend].forEach(legend => {
...
function amenitiesRoadAnalysis(endpoint, layerName, featureFunction, elementId) {
    ...
    fetch(`${BASE_URL_ANALYSIS}/ayodhya/${endpoint}`, {
        method: 'GET'
    })
    .then(response => {
        if (!response.ok) throw new Error('Network response was not ok');
...
$(document).ready(function () {
    $.ajax({
        ...
        success: function (xml) {
            var select = $("#layer");
            const targetLayer = "ANN_Summary:ayodhya_road_net"; // your desired layer
...
if (value_operator === "BETWEEN") {
    ...
    $.getJSON(url, function (data) {
		...
		var table = document.createElement("table");
				...
				caption.style.backgroundColor = "rgb(191 126 42)";
...
function clearAllVectorLayers() {
    ...
    const elementsToHide = [ "road-filter","drain-filter", "filterBox", "query_tab", "popup_road", "tableContainer_summary", "Scoreing_tableContainer",
        "tableContainer_summaryfilter", "summary-table", 'table_data'];
    ...
    const legendIds = [
         'Condition_legend', 'Material_legend', 'Ownership_legend', 'CUS_legend', 'RoadCategory_legend', 'Drain_Type_Legend',
...
function zoomToIndia() {
    ...
    view.setCenter([82.1677, 26.7746]);
    view.setZoom(12);
    view.setRotation(0);
...
function updateSummary(data) {
  ...
  //  console.log("✅ Received Data in updateSummary:", data); // For Debugging
    ...
    const summaryData = {
        ...
        'Total Road Length': { value: `${safeValue('total_length_km').toFixed(2)} km`, onclick: "Ayodhya_Road_Length_Count(); updateNavBarWithFunctionName('Total Road Length');" },
        'Total No. of Roads': { value: safeValue('total_length_count'), onclick: "Ayodhya_Road_Length_Count(); updateNavBarWithFunctionName('Total Road Count');" },
        'Total Ward No.': { value: safeValue('ayo_ward'), },
        'Road Count by Condition': {
            value: `
                Good - <a href="javascript:void(0)" onclick="Ayodhya_Condition_cat('condition','Good'); updateNavBarWithFunctionName('Road Condition :Good');" style="color:green;">${safeValue('condition_count_green')}</a><br>
                Poor - <a href="javascript:void(0)" onclick="Ayodhya_Condition_cat('condition','Poor'); updateNavBarWithFunctionName('Road Condition :Poor');" style="color:red;">${safeValue('condition_count_red')}</a><br>
                Moderate - <a href="javascript:void(0)" onclick="Ayodhya_Condition_cat('condition','Moderate'); updateNavBarWithFunctionName('Road Condition :Moderate');" style="color:yellow;">${safeValue('condition_count_yellow')}</a><br>
            `,
            onclick:'Ayodhya_Condition()'
        },
        'Road Count by Material': {
            value: `
                Bitumen - <a href="javascript:void(0)" onclick="Ayodhya_Material_cat('material','Bitumen'); updateNavBarWithFunctionName('Road Material :Bitumen');" style="color:darkred;">${safeValue('count_bitumen')}</a><br>
                CC - <a href="javascript:void(0)" onclick="Ayodhya_Material_cat('material','CC'); updateNavBarWithFunctionName('Road Material :CC');" style="color:#1ad7b0;">${safeValue('count_cc')}</a><br>
                Interlocking - <a href="javascript:void(0)" onclick="Ayodhya_Material_cat('material','Interlocking'); updateNavBarWithFunctionName('Road Material :Interlocking');" style="color:#2392ed;">${safeValue('count_interlocking')}</a><br>
                BOE - <a href="javascript:void(0)" onclick="Ayodhya_Material_cat('material','BOE'); updateNavBarWithFunctionName('Road Material :BOE');" style="color:#f228ab;">${safeValue('count_boe')}</a><br>
                Kachcha - <a href="javascript:void(0)" onclick="Ayodhya_Material_cat('material','Kachcha'); updateNavBarWithFunctionName('Road Material :Kachcha');" style="color:#6036d0;">${safeValue('count_kachcha')}</a><br>
        ...
        'Road Count by Ownership': {
            value: `
                ANN - <a href="javascript:void(0)" onclick="Ayodhya_Ownership_cat('ownership','ANN'); updateNavBarWithFunctionName('Road Ownership : Ayodhya Nagar Nigam');" style="color:#5aeee5;">${safeValue('count_ann')}</a><br>
                PWD - <a href="javascript:void(0)" onclick="Ayodhya_Ownership_cat('ownership','PWD'); updateNavBarWithFunctionName('Road Ownership : PWD');" style="color:#69e70f;">${safeValue('count_pwd')}</a><br>
                Railway - <a href="javascript:void(0)" onclick="Ayodhya_Ownership_cat('ownership','Railway'); updateNavBarWithFunctionName('Road Ownership : Railway');" style="color:#f16a16;">${safeValue('count_railway')}</a><br>
                Private Road - <a href="javascript:void(0)" onclick="Ayodhya_Ownership_cat('ownership','Private Road'); updateNavBarWithFunctionName('Road Ownership : Private Road');" style="color:#ed2323;">${safeValue('count_pvt')}</a><br>
                NHAI - <a href="javascript:void(0)" onclick="Ayodhya_Ownership_cat('ownership','NHAI'); updateNavBarWithFunctionName('Road Ownership : NHAI');" style="color:#5aeee5;">${safeValue('count_nhai')}</a><br>
                UPSBC Ltd. - <a href="javascript:void(0)" onclick="Ayodhya_Ownership_cat('ownership','UPSBC Ltd.'); updateNavBarWithFunctionName('Road Ownership : UPSBC Ltd.');" style="color:#5aeee5;">${safeValue('count_grid')}</a><br>
                `,
            onclick: 'Ayodhya_Ownership()'
        },
        'Road Count by CUS': {
            value: `
                NA- <a href="javascript:void(0)" onclick="Ayodhya_CUS_cat('cus','NA'); updateNavBarWithFunctionName('Road CUS NA');" style="color: #bad057;">${safeValue('count_cus_na')}</a><br>
        ...
        'Road Count by Category': {
            value: `
                Local Street - <a href="javascript:void(0)" onclick="Ayodhya_TypeSub_cat('category','Local Street'); updateNavBarWithFunctionName('Road Category : Local Street');" style="color: #14cee3;">${safeValue('count_local_street')}</a><br>
                Collector - <a href="javascript:void(0)" onclick="Ayodhya_TypeSub_cat('category','Collector'); updateNavBarWithFunctionName('Road Category : Collector');" style="color: #e63dee;">${safeValue('count_collector')}</a><br>
                Arterial - <a href="javascript:void(0)" onclick="Ayodhya_TypeSub_cat('category','Arterial'); updateNavBarWithFunctionName('Road Category : Arterial');" style="color: #e1ca4c;">${safeValue('count_arterial')}</a><br>
                Sub Arterial - <a href="javascript:void(0)" onclick="Ayodhya_TypeSub_cat('category','Sub Arterial'); updateNavBarWithFunctionName('Road Category : Sub Arterial');" style="color: #83e45c;">${safeValue('count_subarterial')}</a><br>
...
function updateZones(zoneNo, zoneName, data) {
    ...
    const zoneCards = [
...

        {
            ...
            value: `Good <a href="javascript:void(0)" onclick="Ayodhya_Zone_Condition('${zoneNo}','condition','Good'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Condition :Good');" style="color:green;">- ${data.count_green}</a> <br> 
                    Moderate <a href="javascript:void(0)" onclick="Ayodhya_Zone_Condition('${zoneNo}','condition','Moderate'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Condition :Moderate');" style="color:yellow;">- ${data.count_yellow}</a> <br> 
                    Poor <a href="javascript:void(0)" onclick="Ayodhya_Zone_Condition('${zoneNo}','condition','Poor'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Condition :Poor');" style="color:red;">- ${data.count_red}</a><br>

                    `
        },
        {
            title: 'Materials',
            value: `Bitumen <a href="javascript:void(0)" onclick="Ayodhya_Zone_Material('${zoneNo}','material','Bitumen'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Material :Bitumen');" style="color:darkred;">- ${data.bitumen}</a> <br>
                    CC <a href="javascript:void(0)" onclick="Ayodhya_Zone_Material('${zoneNo}','material','CC'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Material :CC');" style="color:#1ad7b0;">- ${data.cc}</a> <br>
                    Interlocking <a href="javascript:void(0)" onclick="Ayodhya_Zone_Material('${zoneNo}','material','Interlocking'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Material :Interlocking');" style="color:#2392ed;">- ${data.interlocking}</a> <br>
                    BOE <a href="javascript:void(0)" onclick="Ayodhya_Zone_Material('${zoneNo}','material','BOE'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Material :BOE');" style="color:#f228ab;">- ${data.boe}</a> <br>
                    Kachcha <a href="javascript:void(0)" onclick="Ayodhya_Zone_Material('${zoneNo}','material','Kachcha'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Material :Kachcha');" style="color:#6036d0;">- ${data.kachcha}</a><br>
                    `

        },
        {
            title: 'Ownership',
            value: `ANN <a href="javascript:void(0)" onclick="Ayodhya_Zone_Ownership('${zoneNo}','ownership','ANN'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership : Ayodhya Nagar Nigam');" style="color:#5aeee5;">- ${data.count_ann}</a> <br>
                    PWD <a href="javascript:void(0)" onclick="Ayodhya_Zone_Ownership('${zoneNo}','ownership','PWD'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership : PWD');" style="color:#69e70f;">- ${data.count_pwd}</a> <br>
                    Private Road <a href="javascript:void(0)" onclick="Ayodhya_Zone_Ownership('${zoneNo}','ownership','Private Road'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership : Private Road');" style="color:#ed2323;">- ${data.count_pvt}</a><br>
                    Railway - <a href="javascript:void(0)" onclick="Ayodhya_Zone_Ownership('${zoneNo}','ownership','Railway'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership : Railway');" style="color:#f16a16;">${data.count_railway}</a><br>
                    NHAI - <a href="javascript:void(0)" onclick="Ayodhya_Zone_Ownership('${zoneNo}','ownership','NHAI'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership : NHAI');" style="color:#5aeee5;">${data.count_nhai}</a><br>
                    UPSBC Ltd. - <a href="javascript:void(0)" onclick="Ayodhya_Zone_Ownership('${zoneNo}','ownership','UPSBC Ltd.'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Ownership : UPSBC Ltd.');" style="color:#5aeee5;">${data.count_grid}</a><br>
                  `
        },
        {
            title: 'CUS',
            value: `
                         NA- <a href="javascript:void(0)" onclick="Ayodhya_Zone_CUS('${zoneNo}','cus','NA'); updateNavBarWithFunctionName('Zone-${zoneNo} Road CUS NA');" style="color: #bad057;">${data.count_cus_na}</a><br>

                        `  },
        {
            title: 'Type Sub Category',
            value: `
                    Local Street - <a href="javascript:void(0)" onclick="Ayodhya_Zone_TypeSub('${zoneNo}','category','Local Street'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Category : Local Street');" style="color: #14cee3;">${data.count_local_street} </a><br>
                    Collector - <a href="javascript:void(0)" onclick="Ayodhya_Zone_TypeSub('${zoneNo}','category','Collector'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Category : Collector');" style="color: #e63dee;">${data.count_collector}</a><br>
                    Arterial - <a href="javascript:void(0)" onclick="Ayodhya_Zone_TypeSub('${zoneNo}','category','Arterial'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Category : Arterial');" style="color: #e1ca4c;">${data.count_arterial}</a><br>
                    Sub Arterial - <a href="javascript:void(0)" onclick="Ayodhya_Zone_TypeSub('${zoneNo}','category','Sub Arterial'); updateNavBarWithFunctionName('Zone-${zoneNo} Road Category : Sub Arterial');" style="color: #83e45c;">${data.count_subarterial}</a><br>
        ...
        .then(responseData => {
            if (responseData.status) {
                ...
                const details = [
                    ...
                    {
                        title: 'Total Road Length',
                        value: `<a href="javascript:void(0)" onclick="Ayodhya_Zone_no('ward_no', '${wardNo}'); updateNavBarWithFunctionName('Ward-${wardNo} Total Road Length');" style="color:black;">${responseData.length_km.toFixed(2)} km</a>`
                    },
                    {
                        title: 'Total No. of Roads',
                    ...
                    {
                        title: 'Road Condition',
                        value: `Good - <a href="javascript:void(0)" onclick="Ayodhya_Ward_Condition('${wardNo}', 'condition', 'Good'); updateNavBarWithFunctionName('Ward-${wardNo} Road Condition :Good');" style="color:green;">${responseData.count_green}</a> 
                                <br> Moderate - <a href="javascript:void(0)" onclick="Ayodhya_Ward_Condition('${wardNo}', 'condition', 'Moderate'); updateNavBarWithFunctionName('Ward-${wardNo} Road Condition :Moderate');" style="color:yellow;">${responseData.count_yellow}</a> 
                                <br> Poor - <a href="javascript:void(0)" onclick="Ayodhya_Ward_Condition('${wardNo}', 'condition', 'Poor'); updateNavBarWithFunctionName('Ward-${wardNo} Road Condition :Poor');" style="color:red;">${responseData.count_red}</a>
                                `

                    },
                    {
                        title: 'Materials',
                        value: `Bitumen - <a href="javascript:void(0)" onclick="Ayodhya_Ward_Material('${wardNo}', 'material', 'Bitumen'); updateNavBarWithFunctionName('Ward-${wardNo} Road Material :Bitumen');" style="color:darkred;">${responseData.count_bitumen}</a><br> 
                        CC - <a href="javascript:void(0)" onclick="Ayodhya_Ward_Material('${wardNo}', 'material', 'CC'); updateNavBarWithFunctionName('Ward-${wardNo} Road Material :CC');" style="color:cyan;">${responseData.count_cc}</a><br> 
                        Interlocking - <a href="javascript:void(0)" onclick="Ayodhya_Ward_Material('${wardNo}', 'material', 'Interlocking'); updateNavBarWithFunctionName('Ward-${wardNo} Road Material :Interlocking');" style="color:blue;">${responseData.count_interlocking}</a><br> 
                        BOE - <a href="javascript:void(0)" onclick="Ayodhya_Ward_Material('${wardNo}', 'material', 'BOE'); updateNavBarWithFunctionName('Ward-${wardNo} Road Material :BOE');" style="color:pink;">${responseData.count_boe}</a><br>
                        Kachcha - <a href="javascript:void(0)" onclick="Ayodhya_Ward_Material('${wardNo}', 'material', 'Kachcha'); updateNavBarWithFunctionName('Ward-${wardNo} Road Material :Kachcha');" style="color:purple;">${responseData.count_kachcha}</a><br>
                
                         `
                    },
                    {
                        title: 'Ownership',
                        value: `ANN - <a href="javascript:void(0)" onclick="Ayodhya_Ward_Ownership('${wardNo}', 'ownership','ANN'); updateNavBarWithFunctionName('Ward-${wardNo} Road Ownership : Ayodhya Nagar Nigam');" style="color:#5aeee5;">${responseData.count_ann}</a><br>
                                PWD <a href="javascript:void(0)" onclick="Ayodhya_Ward_Ownership('${wardNo}','ownership','PWD'); updateNavBarWithFunctionName('Ward-${wardNo} Road Ownership : PWD');" style="color:#69e70f;">- ${responseData.count_pwd}</a> <br>
                                Private Road <a href="javascript:void(0)" onclick="Ayodhya_Ward_Ownership('${wardNo}','ownership','Private Road'); updateNavBarWithFunctionName('Ward-${wardNo} Road Ownership : Private Road');" style="color:#ed2323;">- ${responseData.count_pvt}</a><br>
                                Railway - <a href="javascript:void(0)" onclick="Ayodhya_Ward_Ownership('${wardNo}','ownership','Railway'); updateNavBarWithFunctionName('Ward-${wardNo} Road Ownership : Railway');" style="color:#f16a16;">${responseData.count_railway}</a><br>
                                NHAI - <a href="javascript:void(0)" onclick="Ayodhya_Ward_Ownership('${wardNo}','ownership','NHAI'); updateNavBarWithFunctionName('Ward-${wardNo} Road Ownership : NHAI');" style="color:#5aeee5;">${responseData.count_nhai}</a><br>
                                UPSBC Ltd. - <a href="javascript:void(0)" onclick="Ayodhya_Ward_Ownership('${wardNo}','ownership','UPSBC Ltd.'); updateNavBarWithFunctionName('Ward-${wardNo} Road Ownership : UPSBC Ltd.');" style="color:#5aeee5;">${responseData.count_grid}</a><br>
                                `
                    },
                    {
                        title: 'CUS',
                        value: `
                                    ...
                                    NA- <a href="javascript:void(0)" onclick="Ayodhya_Ward_CUS('${wardNo}','cus','NA'); updateNavBarWithFunctionName('Ward-${wardNo} Road CUS NA');" style="color: #bad057;">${responseData.count_cus_na}</a><br>
           
                                    `  },
                    {
                        title: 'Category',
                        value: `
                                Local Street - <a href="javascript:void(0)" onclick="Ayodhya_Ward_TypeSub('${wardNo}','category','Local Street'); updateNavBarWithFunctionName('Ward-${wardNo} Road Category : Local Street');" style="color: #14cee3;">${responseData.count_local_street} </a><br>
                                Collector - <a href="javascript:void(0)" onclick="Ayodhya_Ward_TypeSub('${wardNo}','category','Collector'); updateNavBarWithFunctionName('Ward-${wardNo} Road Category : Collector');" style="color: #e63dee;">${responseData.count_collector}</a><br>
                                Arterial - <a href="javascript:void(0)" onclick="Ayodhya_Ward_TypeSub('${wardNo}','category','Arterial'); updateNavBarWithFunctionName('Ward-${wardNo} Road Category : Arterial');" style="color: #e1ca4c;">${responseData.count_arterial}</a><br>
                                Sub Arterial - <a href="javascript:void(0)" onclick="Ayodhya_Ward_TypeSub('${wardNo}','category','Sub Arterial'); updateNavBarWithFunctionName('Ward-${wardNo} Road Category : Sub Arterial');" style="color: #83e45c;">${responseData.count_subarterial}</a><br>
...
//         console.log(`✅ Rendered ${data.length} rows successfully`);
...
function Table_Row_and_Layer_highlight(data) {
    ...
    console.log(`🚀 Starting lazy rendering for ${data.length} rows...`);
    ...
    function renderNextBatch() {
        ...
        for (let i = currentIndex; i < end; i++) {
            ...
            row.innerHTML = `
                ...
                <td>${item.carriage_w || "N/A"}</td>
                <td>${item.material || "N/A"}</td>
                <td>${item.length_km.toFixed(2) || "N/A"}</td>
                <td>${item.condition || "N/A"}</td>
                <td>${item.yoc || "N/A"}</td>
                <td>${item.cus || "N/A"}</td>
        ...
        if (currentIndex < data.length) {
            ...
            console.log(`✅ Fully rendered ${data.length} rows`);
        }
    }
...
function highlightFeatureOnMap(gisId) {
    let wfsUrl = `${GEOSERVER_BASE_URL}/wfs?service=WFS&version=1.1.0&request=GetFeature
        &typename=ANN_Summary:ayodhya_road_net
...
<span id="popupCloseBtn"
  ...
  <div class="row-line"><strong>Length (Km) :</strong> ${
  	      properties.length_km !== null && properties.length_km !== undefined
  	          ...
  	          : 'N/A'
  	  }</div>  <div class="row-line"><strong>Road Name :</strong> ${properties.road_name || 'N/A'}</div>
</div>
...
function Ayodhya_Road_Length_Count() {
    removeCurrentLayer();
    clearVectorLayers();
    ...
    currentLayer = new ol.layer.Image({
        ...
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'ANN_Summary:ayodhya_road_net',
    ...
    //overlays.getLayers().push(ANN_Ward_Boundary);
    map.addLayer(currentLayer);
    ANN_Road_popup();

}

function Ayodhya_Condition(cqlFilter) {
    ...
    currentLayer = new ol.layer.Image({
        ...
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'ANN_Summary:ayodhya_road_net_condition',
...
function Ayodhya_Ownership(cqlFilter) {
    ...
    currentLayer = new ol.layer.Image({
        ...
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'ANN_Summary:ayodhya_road_net_own',
                'CQL_FILTER': cqlFilter,
            },
            ratio: 1,
            serverType: 'geoserver'
...
function Ayodhya_Material(cqlFilter) {
    ...
    currentLayer = new ol.layer.Image({
        ...
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'ANN_Summary:ayodhya_road_net_material',
                'CQL_FILTER': cqlFilter,
            },
            ratio: 1,
            serverType: 'geoserver'
...
function Ayodhya_Types(cqlFilter) {
    ...
    currentLayer = new ol.layer.Image({
        ...
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'ANN_Summary:ayodhya_road_net_category',
                'CQL_FILTER': cqlFilter,
            },
            ratio: 1,
            serverType: 'geoserver'
...
$(document).ready(function () {
    ...
    var wfsParams = {
        ...
        typeName: 'ANN_Summary:ayodhya_road_net',   // Replace with your WFS layer typename
    ...
    const url = `${GEOSERVER_BASE_URL}/wfs?service=WFS&version=1.0.0&request=GetFeature&typeName=ANN_Summary:ayodhya_road_net&outputFormat=application/json`;
...
function Ayodhya_Ward_no(column, value) {
    ...
    fetchANN_ALLFilteredData(column, value);  // Optional: You can enhance this function too if needed
}
...
function Ayodhya_Condition_cat(column, value) {
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'ANN_Summary:ayodhya_road_net_condition',   // Same layer for all filters
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Ayodhya_Material_cat(column, value) {
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'ANN_Summary:ayodhya_road_net_material',  // Same layer for all filters
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Ayodhya_Ownership_cat(column, value) {
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'ANN_Summary:ayodhya_road_net_own',   // Same layer for all filters
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Ayodhya_CUS_cat(column, value) {
    removeCurrentLayer();
    clearVectorLayers();
    // Define dynamic CQL filter
    let cqlFilter = `(${column}='${value}' OR ${column} ILIKE '%/${value}' OR ${column} ILIKE '${value}/%' OR ${column} ILIKE '%/${value}/%' OR ${column} ILIKE '${value}')`;
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'ANN_Summary:ayodhya_road_net_cus',   // Same layer for all filters
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Ayodhya_TypeSub_cat(column, value) {
    removeCurrentLayer();
    clearVectorLayers();
    // Define dynamic CQL filter
    let cqlFilter = `(${column}='${value}' OR ${column} ILIKE '%/${value}' OR ${column} ILIKE '${value}/%' OR ${column} ILIKE '%/${value}/%' OR ${column} ILIKE '${value}')`;
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': '	ANN_Summary:ayodhya_road_net_category',   // Same layer for all filters
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function loadPageData(isFirstBatch = false) {
    ...
    fetch(`${BASE_URL}/getData?column=${activeColumn}&value=${activeValue}&page=${currentPage}&size=10000`, {
        ...
        .catch(err => {
            ...
            isLoading = false;
            if (progressLabel) progressLabel.innerText = "❌ Error loading data.";
            if (spinner) spinner.style.display = 'none';
        });
}
...
function appendTableRows(data) {
    ...
    if (!dataTableBody_summary) {
        console.error("❌ Element with id 'dataBody_summary' not found!");
        return;
    }
...
function Ayodhya_Zone_Material(zone_no, column, value) {
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'ANN_Summary:ayodhya_road_net_material',   // Main layer for filtering
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Ayodhya_Zone_Ownership(zone_no, column, value) {
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'ANN_Summary:ayodhya_road_net_own',   // Main layer for filtering
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Ayodhya_Zone_CUS(zone_no, column, value) {
    removeCurrentLayer();
    clearVectorLayers();
    // Ensure column is valid to prevent errors
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'ANN_Summary:ayodhya_road_net_cus',   // Main layer for filtering
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Ayodhya_Zone_TypeSub(zone_no, column, value) {
    removeCurrentLayer();
    clearVectorLayers();
    // Ensure column is valid to prevent errors
    let validColumns = ["condition", "category", "material", "ownership", "cus",];
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'ANN_Summary:ayodhya_road_net_category',   // Main layer for filtering
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Ayodhya_Ward_Condition(ward_no, column, value) {
    ...
    currentLayer = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: `${GEOSERVER_BASE_URL}/wms`,
            params: {
                'LAYERS': 'ANN_Summary:ayodhya_road_net_condition', // Main layer for filtering
                'CQL_FILTER': cqlFilter, // Apply dynamic filtering
                'TILED': true
            },
            ratio: 1,
...
function Ayodhya_Ward_CUS(ward_no, column, value) {
    removeCurrentLayer();
    clearVectorLayers();
...
function Ayodhya_Ward_TypeSub(ward_no, column, value) {
    removeCurrentLayer();
    clearVectorLayers();
    // Ensure column is valid to prevent errors
    let validColumns = ["condition", "category", "material", "ownership", "cus"];
...
downloadMenu.addEventListener("click", (e) => {
  ...
  case "print": {
    ...
    setTimeout(() => {
      ...
      const pixelShift = 300;          // tune once (300–500 typical)
...
// ================= KML =================
/*function downloadTableAsKML(table, stamp) {
  ...
  const headers = Array.from(table.querySelectorAll("thead th"))
    .map(th =>
      th.innerText
        .trim()
        .toLowerCase()
        .replace(/\([^)]*\)/g, "")     // remove (km), (meter)
        .replace(/[^a-z0-9]+/g, "_")   // symbols → _
        .replace(/^_|_$/g, "")
    );
  const geomIndex = headers.indexOf("geom");
  if (geomIndex === -1) {
    alert("GEOM column not found");
    return null;
  }
  const getVal = (tr, name) => {
    const key = name.toLowerCase().replace(/[^a-z0-9]+/g, "_");
    const idx = headers.indexOf(key);
    return idx !== -1 ? tr.children[idx].innerText.trim() : "";
  };
  let kml = `<?xml version="1.0" encoding="UTF-8"?>\n`;
  kml += `<kml xmlns="http://www.opengis.net/kml/2.2">\n<Document>\n`;
  const rows = table.querySelectorAll("tbody tr");
  rows.forEach((tr, i) => {
    const geomText = tr.children[geomIndex].innerText.trim();
    if (!geomText.startsWith("MULTILINESTRING")) return;
    // --- Robust MULTILINESTRING parsing ---
    const lines = geomText
    ...
    lines.forEach((line, j) => {
      ...
      kml += `<Placemark>\n`;
      kml += `<name>${getVal(tr, "road_name") || `Feature ${i + 1}`}</name>\n`;
      kml += `
<ExtendedData>
  <Data name="road_id"><value>${getVal(tr, "road_id")}</value></Data>
  <Data name="road_name"><value>${getVal(tr, "road_name")}</value></Data>
   <Data name="material"><value>${getVal(tr, "material")}</value></Data>
  <Data name="ownership"><value>${getVal(tr, "ownership")}</value></Data>
  <Data name="cus"><value>${getVal(tr, "cus")}</value></Data>
</ExtendedData>
`;
      kml += `
<LineString>
...
 <coordinates>${coordinates}</coordinates></LineString></Placemark>\n`;    });  });
  kml += `</Document>\n</kml>`; 
 return kml;
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const filters = ['ward', 'ownership', 'category', 'material'];
...
function determineLayer() {
    if (selectedFilters.material) {
        return 'ANN_Summary:ayodhya_road_net_material';
    } else if (selectedFilters.type) {
        return 'ANN_Summary:ayodhya_road_net_category';
    } else if (selectedFilters.ownership) {
        return 'ANN_Summary:ayodhya_road_net_own';
    }
    return 'ANN_Summary:ayodhya_road_net_category';
}
...
function determineLayer() {
    ...
    if (selectedFilters.material) {
        ...
        document.getElementById('Ownership_legend').style.display = 'none';
        document.getElementById('Material_legend').style.display = 'none';
        return 'ANN_Summary:ayodhya_road_net_condition';
    }
    return 'ANN_Summary:ayodhya_road_net';
}
function updateMapLayer(layer, cqlFilter) {
    removeCurrentLayer();
    clearVectorLayers();
...
function Ayodhya_Type_Drain(column, value) {
   ...
   // ANN_Road();
    ...
    hideElementIds = (['Material_legend','type_legend','Condition_legend','Ownership_legend',
		//'Priority_legend','Scoreing_tableContainer',
...
function Ayodhya_Status_Drain(column, value) {
 ...
 //   ANN_Road();
    ...
    hideElementIds = (['Material_legend','type_legend','Condition_legend','Ownership_legend', //'Priority_legend','Scoreing_tableContainer',
...
function showSelectedRoadsLayer2() {

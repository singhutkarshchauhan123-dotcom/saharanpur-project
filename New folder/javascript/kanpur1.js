        ...
        .then(data => {
            if (data.status && data.data.length > 0) {
                ...
                data.data.forEach(zone => {
                    const option = document.createElement('option');
                    option.value = zone.zone_no;      // Set the zone number as value
                    option.textContent = zone.zone_no; // Display the zone number (you can change this to zone name if needed)
                    zoneDropdown.appendChild(option);
                });
            } else {
                console.error('No zones found.');
    ...
    roadsData.forEach((item, index) => {
        ...
        if (wkt && wkt.startsWith("MULTILINESTRING")) {
            try {
                ...
                // ✅ Set properties including a custom tag
    // ✅ Set properties including a custom tag
      feature.setProperties({
        ...item,
        popup_enabled: true // ✅ Only these features trigger the popup
      });
...
const roadPopupContainer = document.getElementById('road-popup-container');
const roadPopupContent = document.getElementById('road-popup-content');
const roadPopupCloser = document.getElementById('road-popup-closer');

const roadPopupOverlay = new ol.Overlay({
...
roadPopupCloser.onclick = function () {
  ...
  roadPopupContainer.style.display = 'none';
  return false;
};
...
map.on('singleclick', function (evt) {
  ...
  map.forEachFeatureAtPixel(evt.pixel, function (feature) {
    if (feature.get('popup_enabled')) {
      found = true;

      const coordinate = feature.getGeometry().getClosestPoint(evt.coordinate);

      console.log("Feature clicked:", feature.getProperties());
      console.log("Popup coordinate:", coordinate);

      roadPopupContent.innerHTML = `<button id="confirm-road-selection">Confirm Selection</button>`;
    //   roadPopupContainer.style.display = 'block';
      roadPopupOverlay.setPosition(coordinate);
      roadPopupContainer.style.display = 'block';


      setTimeout(() => {
        const btn = document.getElementById('confirm-road-selection');
        if (btn) {
          btn.onclick = function () {
            console.log("✅ Road confirmed:", feature.getProperties());
            roadPopupOverlay.setPosition(undefined);
            roadPopupContainer.style.display = 'none';
          };
        }
